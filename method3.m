% method3_training+DD_equalizer_16apsk.m
% Method 3：Training LMS + Decision-Directed LMS Equalizer for 16-APSK
% Independent version
...
%% ============================================================
% METHOD 3: LS INITIALIZED + TRAINING LMS + DECISION-DIRECTED LMS
%% ============================================================
...
% Step 1: LS equalizer as initial solution
[f_ls, Jmin] = equalize_design(z_train, s_train, ...
    equalizer_order, equalizer_delta);

% Step 2: Training LMS fine-tuning
f_lms = equalize_train_lms_refine(z_train, s_train, ...
    f_ls, equalizer_order, equalizer_delta, mu_train, num_epoch);

% Step 3: Apply DD-LMS on raw received full sequence
[z_eq_all, f_final] = equalize_dd_lms_full(z, f_lms, ...
    N_train, length(m), equalizer_order, equalizer_delta, ...
    mu_dd, CONSTELLATION);

% Step 4: Extract message part
...
xlabel('I');
ylabel('Q');
title('Method 3: Training LMS + Decision-Directed LMS Equalized 16-APSK Symbols');
...
fprintf('\n========== Method 3: Training LMS + Decision-Directed LMS ==========\n');
...
function f = equalize_train_lms(z_train, s_train, n, delta, mu)
    % Training-based complex LMS equalizer
    %
    % z_train : received training symbols
    % s_train : known transmitted training symbols
    % n       : equalizer order
    % delta   : decision delay
    % mu      : LMS step size
    ...
    for i = n+1 : length(z_train)

        rr = z_train(i:-1:i-n+1).';

        y = f.' * rr;

        if i - delta >= 1 && i - delta <= length(s_train)
...
function f = equalize_train_lms_refine(z_train, s_train, f_init, n, delta, mu, num_epoch)
    % LS-initialized training LMS equalizer
    %
    % Start from the LS equalizer result, then use LMS to fine-tune it.
    ...
    for ep = 1:num_epoch
        for k = n+1 : length(z_train)

            if k - delta < 1 || k - delta > length(s_train)
                continue;
            end
            ...
            % Desired training symbol
            d = s_train(k-delta);

            % Error
            e = d - y;

            % Normalized complex LMS update
            norm_x = x' * x + 1e-6;
            f = f + mu * e * conj(x) / norm_x;
        end
    end
end


function [z_eq_all, f_out] = equalize_dd_lms_full(z, f_init, ...
    N_train, N_data, n, delta, mu, CONSTELLATION)

    % Decision-directed LMS applied on the raw received symbol sequence.
    % Important:
    % Do not apply DD-LMS on already equalized data.
    % It should use the original received z sequence.

    z = z(:).';
    f_out = f_init(:);

    z_padded = [z, zeros(1, n + delta)];
    z_eq_all = zeros(1, length(z));
    ...
    for k = n+1 : length(z)
        ...
        if out_index >= 1 && out_index <= length(z)
            z_eq_all(out_index) = y;
        end

        % Only update during data part
        if out_index > N_train && out_index <= N_train + N_data

            % nearest 16-APSK constellation point
            [~, idx] = min(abs(y - CONSTELLATION));
            d_hat = CONSTELLATION(idx);

            e = d_hat - y;

            % very small normalized DD-LMS update
            norm_x = x' * x + 1e-6;
            f_out = f_out + mu * e * conj(x) / norm_x;
        end
    end
end