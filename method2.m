% method2_lms_equalizer_16apsk.m
% Method 2：Training-based LMS Equalizer for 16-APSK
% Independent version
...
%% ============================================================
% METHOD 2: LS INITIALIZED + TRAINING LMS EQUALIZER
%% ============================================================

equalizer_order = 15;
equalizer_delta = 8;

mu_lms = 0.00005;       % smaller step size
num_epoch = 5;          % repeat training several times

% Step 1: Use LS equalizer as a good initial solution
[f_ls, Jmin] = equalize_design(z_train, s_train, ...
    equalizer_order, equalizer_delta);

% Step 2: Fine-tune LS equalizer using training-based LMS
f_lms = equalize_train_lms_refine(z_train, s_train, ...
    f_ls, equalizer_order, equalizer_delta, mu_lms, num_epoch);

% Step 3: Apply refined equalizer to full received sequence
z_eq_all = equalize_apply_nofig(z, f_lms, equalizer_delta);
...
xlabel('I');
ylabel('Q');
title('Method 2: Training-based LMS Equalized 16-APSK Symbols');
...
fprintf('\n========== Method 2: Training-based LMS Equalizer ==========\n');
...
function f = equalize_train_lms_refine(z_train, s_train, f_init, n, delta, mu, num_epoch)
    ...
    %
    % This version does not start from zero.
    % It starts from the LS equalizer and only fine-tunes the coefficients.
    ...
    for ep = 1:num_epoch
        for k = n+1 : length(z_train)
            ...
            % received vector, same structure as LS equalizer
            x = z_train(k:-1:k-n).';
