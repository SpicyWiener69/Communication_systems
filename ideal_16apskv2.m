...
[symbols, num_symbols, tx_passband, t] = transmit(str, oversamp, pulse_half_len, carrier_freq, symbol_period, 0.3);
...
function [symbols, num_symbols, tx_passband, t] = transmit(str, oversamp, pulse_half_len, carrier_freq, symbol_period,timing_offset, do_plot)
    ...
    % Time vector
    t = (1/oversamp : 1/oversamp : length(tx_i)/oversamp) * symbol_period + timing_offset;
...
function [soft_decisions, hard_decisions] = receive(tx_passband, t, oversamp, pulse_half_len, carrier_freq, num_symbols, do_plot)
   ...
   % [soft_decisions ,offset_history]  = timing_recovery_DD(baseband, num_symbols, oversamp, pulse_half_len);
...
function [sym_estimates, offset_history] = timing_recovery_DD(x, n, oversamp, pulse_half_len,do_plot)
    % TIMING_RECOVERY_DD  Decision-directed timing recovery via gradient descent.
    %
    % Inputs:
    %   x              - received signal after matched filter (oversampled)
    %   n              - number of symbols expected
    %   oversamp       - oversampling factor (samples per symbol), e.g. 2
    %   pulse_half_len - half-length of pulse shape in symbols, e.g. 50
    %
    % Outputs:
    %   sym_estimates  - recovered symbol values xs[k]
    %   offset_history - timing offset tau at each symbol iteration

    if nargin < 4, do_plot = true; end
    step_size  = 0.01;   % mu: gradient step (larger = faster but may diverge)
    deriv_step = 0.1;    % delta: small shift for numerical derivative of x
    
    % --- Constellation alphabet for 4-PAM hard decisions ---
    alphabet = [-3, -1, 1, 3];
    
    % --- Initialize ---
    % Start after initial filter transient to avoid edge effects
    sample_pos     = pulse_half_len * oversamp + 1;
    timing_offset  = 0;              % initial timing correction guess
    sym_estimates  = zeros(1, n);
    offset_history = zeros(1, n);
    k = 0;                           % symbol counter
    
    % --- Main loop: iterate over each symbol period ---
    while sample_pos < length(x) - 2 * pulse_half_len * oversamp
    
        k = k + 1;
    
        % 1. Interpolate received signal at current timing estimate
        x_now   = interpsinc(x, sample_pos + timing_offset,              pulse_half_len);
    
        % 2. Interpolate slightly right and left for numerical derivative
        x_right = interpsinc(x, sample_pos + timing_offset + deriv_step, pulse_half_len);
        x_left  = interpsinc(x, sample_pos + timing_offset - deriv_step, pulse_half_len);
    
        % 3. Estimate slope: dx/d(tau) at current position
        slope = x_right - x_left;
    
        % 4. Hard decision: snap to nearest 4-PAM symbol
        decision = quantalph(x_now, alphabet);
    
        % 5. DD update: nudge timing_offset to reduce (decision - x_now)^2
        %    gradient of cluster variance w.r.t. tau:
        %    d/dtau (decision - x(tau))^2 = -2*(decision - x_now)*slope
        timing_offset = timing_offset + step_size * slope * (decision - x_now);
    
        % 6. Advance to next symbol, save state
    ...
    % Trim unused trailing zeros
    sym_estimates  = sym_estimates(1:k);
    offset_history = offset_history(1:k);
    
    if do_plot == true
        % plot symbol estimates
        figure();
        plot(sym_estimates, '.');
        title('Symbol Estimates');
        xlabel('Symbol Index');
        ylabel('Amplitude');
        grid on;
        
        % plot timing offset estimates
        figure();
        plot(offset_history);
