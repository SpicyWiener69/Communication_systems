...
% Add guard symbols to protect the end of the message
...
z_eq_all = equalize_apply(z, f, equalizer_delta);

% Only extract the real message part, not the guard symbols
...
function z_eq = equalize_apply(z, f, delta)
    ...
    % Compensate equalizer decision delay.
    % y_full(k + delta) estimates the symbol around z(k).
...
function p = srrc_pulse(alpha, span, sps)
    % SRRC pulse without toolbox
    ...
    for k = 1:length(t)
        tau = t(k);
        if abs(tau) < 1e-8
            p(k) = 1 - alpha + 4*alpha/pi;
        elseif alpha ~= 0 && abs(abs(tau) - 1/(4*alpha)) < 1e-8
            p(k) = (alpha/sqrt(2)) * ...
                ((1 + 2/pi) * sin(pi/(4*alpha)) + ...
                 (1 - 2/pi) * cos(pi/(4*alpha)));
        else
            numerator = sin(pi*tau*(1-alpha)) + ...
                        4*alpha*tau*cos(pi*tau*(1+alpha));
            denominator = pi*tau*(1 - (4*alpha*tau)^2);
